...
public class Testcase6 {
    ...
    public void ncltaddcase() throws InterruptedException {
        ...
        checkNumericField(CPIBNo, "abcd", "CPIB Number");
    ...
    private void checkNumericField(WebElement field, String valueToEnter, String fieldName) {
        field.clear();
        field.sendKeys(valueToEnter);
        String enteredValue = field.getAttribute("value");

        if (enteredValue.matches(".*[a-zA-Z].*")) {
            System.out.println(" 6 ) Failed - Alphabets can be entered in " + fieldName);
        } else {
            System.out.println(" 6) Passed - Alphabets cannot be entered in " + fieldName);
        }
    }
}
