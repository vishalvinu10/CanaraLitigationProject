...
public class Testcase7 {
	...
	private static final String SCROLL_SCRIPT = "arguments[0].scrollIntoView({block: 'nearest', inline: 'start'});";
	 private String generatedDiaryNumber;
    ...
    public void ncltaddcase() throws InterruptedException {
        ...
        // Check special characters for 'provsion' field
        WebElement provsion = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='TxtNCLTProvision']")));
        provsion.sendKeys("@#%&!");
        if (provsion.getAttribute("value").contains("@#%&!")) {
            System.out.println(" 7) provsion field failed - special characters can be entered");
        } else {
            System.out.println("provsion field passed - special characters cannot be entered");
        }

        // Check special characters for 'LEInumber' field
        WebElement LEInumber = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='TxtNCLTLEINo']")));
        LEInumber.sendKeys("@#%&!");
        if (LEInumber.getAttribute("value").contains("@#%&!")) {
            System.out.println("LEInumber field failed - special characters can be entered");
        } else {
            System.out.println("LEInumber field passed - special characters cannot be entered");
        }
        ...
        // Check special characters for 'respontantname' field
        WebElement respontantname = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='TxtNCLTRespondent']")));
        respontantname.sendKeys("@#%&!");
        if (respontantname.getAttribute("value").contains("@#%&!")) {
            System.out.println("respontantname field failed - special characters can be entered");
        } else {
            System.out.println("respontantname field passed - special characters cannot be entered");
            
            // Enter "rajkumar" into the respontantname field
            respontantname.clear();  // Clear the field before entering a new value
            respontantname.sendKeys("rajkumar");
        ...
        WebElement roleofbank = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("/html[1]/body[1]/div[1]/div[2]/div[1]/div[1]/div[2]/div[2]/div[1]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/div[23]/fieldset[1]/select[1]")));
        ...
        // Check special characters for 'diarynoElement' field
        WebElement diarynoElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='TxtNCLTDiaryNo']")));
        diarynoElement.sendKeys("@#%&!");
        if (diarynoElement.getAttribute("value").contains("@#%&!")) {
            System.out.println("diarynoElement field failed - special characters can be entered");
        } else {
            System.out.println("diarynoElement field passed - special characters cannot be entered");
        }
        ...
        // Check special characters for 'CPIBNo' field
        WebElement CPIBNo = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='TxtCPIBNo']")));
        CPIBNo.sendKeys("@#%&!");
        if (CPIBNo.getAttribute("value").contains("@#%&!")) {
            System.out.println("CPIBNo field failed - special characters can be entered");
        } else {
            System.out.println("CPIBNo field passed - special characters cannot be entered");
        }
